    ...
    <div className="p-3 sm:p-4 lg:p-6 flex gap-4">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 mb-6">
          ...
          <button onClick={openNew} className="flex items-center gap-2 px-4 py-2 min-h-11 bg-orange-500 text-white rounded-xl hover:bg-orange-600 font-medium">
        ...
        <div className="relative mb-4">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="ค้นหาชื่อ หรือเบอร์โทร..."
            className="w-full pl-9 pr-3 sm:pr-4 py-2 min-h-11 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-orange-300" />
        </div>
          ...
          {filtered.length === 0 ? (
            ...
            return (
              <div
                ...
                className={clsx('bg-white rounded-2xl p-3 sm:p-4 shadow-sm border cursor-pointer transition-all hover:border-orange-300 hover:shadow-md min-h-44',
              ...
              >
                <div className="flex items-start justify-between">
                  ...
                  <div className="flex gap-1">
                    <button onClick={e => { e.stopPropagation(); openEdit(m) }} className="p-2 min-h-10 min-w-10 text-gray-400 hover:text-orange-500 rounded-lg"><Pencil size={14} /></button>
                    <button onClick={e => { e.stopPropagation(); remove(m.id) }} className="p-2 min-h-10 min-w-10 text-gray-400 hover:text-red-500 rounded-lg"><Trash2 size={14} /></button>
                  </div>
                </div>
      ...
      {selected && (
        <div className="w-72 shrink-0">
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-3 sm:p-5 sticky top-6">
            ...
            <button onClick={() => setSelected(null)} className="mt-4 w-full py-2 min-h-11 text-sm text-gray-500 hover:bg-gray-50 rounded-xl">ปิด</button>
          </div>
        </div>
      )}
              ...
              {[
                ...
                <div key={f.key}>
                  ...
                  <input value={(form as any)[f.key]} onChange={e => setForm({ ...form, [f.key]: e.target.value })}
                    ...
                    className="w-full border border-gray-200 rounded-xl px-3 py-2 min-h-11 text-sm focus:outline-none focus:ring-2 focus:ring-orange-300" />
                </div>
              ))}
              <button onClick={save} disabled={saving || !form.name || !form.phone}
                className="w-full py-3 min-h-12 bg-orange-500 hover:bg-orange-600 disabled:bg-gray-200 text-white rounded-xl font-semibold transition-colors">
